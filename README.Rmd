...
The goal of this talk is to introduce the use of tools that simplify the development of packages in R. It's a free 

El objetivo de este workshop es introducir el uso de herramientas que simplifican la creación de paquetes en R. It is a free and summarized interpretation of the course prepared by:
...
usethis::use_course("http://bit.ly/30kL8QD") ### CAMBIAR
```
...
- For a package to be accepted in CRAN it must have neither errors nor warnings, and the notes are inspected by the evaluators (in general there is a note for packages that are uploaded for the first time, and there should be no others than this one).
...
To create the license we can use `usethis::use_mit_license()` function
...
The test coverage indicate the proportion of the code that is covered bt tests. Not all code is easily testable, but a high coverage is a guarantee that there are no hidden problems. (in the original notes there is a detail explanation on how to check coverage)
...
For this to work we have to have previously synchronized our computer with a GitHub session.

For example, for me, these commands created the following repo: [https://github.com/DiegoKoz/tstpckg](https://github.com/DiegoKoz/tstpckg) ###CAMBIAR
